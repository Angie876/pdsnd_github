...
def get_filters():
    """
    Asks the user to specify a city, month, and day to analyze and filter.
    ...
    # Validating user input for month
    while(True):
...
def load_data(city, month, day):
    ...
    # loads data file into a dataframe for use in the program
    df = pd.read_csv(CITY_DATA[city])

    df['Start Time'] = pd.to_datetime(df['Start Time'], errors='coerce')
    
    # Months values take 1 through 12
    df['month'] = df['Start Time'].dt.month  
    
    #  range of the day of week is 1 through 7
...
def trip_duration_stats(df):
    ...
    # Converting the time in seconds to time in days.
